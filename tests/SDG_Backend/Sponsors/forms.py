## Sponsor forms under the Sponsors folders
...
from SDG_Backend.models import Sponsor
...
class RegistrationForm(FlaskForm):
    ...
    def check_email(self,field):
        ...
        if Sponsor.query.filter_by(email=field.data).first():
            raise ValidationError('Your email has been registered!')

    def check_username(self,field):

        if Sponsor.query.filter_by(username=field.data).first():
